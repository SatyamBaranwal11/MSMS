...
<!-- <form name="submit" method="post" enctype="multipart/form-data" action="display.php">  -->
...
<tr>
  ...
  <tr align="center">
    <!-- <td colspan="2"><button type="submit" name="submit" onclick="submitForm()" class="btn btn-az-primary pd-x-20">Submit</button></td> -->
...
if(isset($_POST['submit'])){
  ...
  // echo "<script>console.log('$status');</script>";
    ...
    try {
        ...
        // $mail->addAddress('karmur9487@gmail.com', 'fake fake'); 
        $mail->addAddress($userEmail, ' '); // Add the user's email address here

        $mail->isHTML(true);                              
        $mail->Subject = 'Appointment Status';

        // Set the email body based on the selected status
        if($status == 1) {
            $mail->Body    = $remark. '<br>  Your appointment is accepted <br> Your Services are '.$services. ' <br> your appointment number is '.$appo;
        } else if($status == 2) {
            $mail->Body = $remark. '<br> Your appointment is rejected.';
        }
